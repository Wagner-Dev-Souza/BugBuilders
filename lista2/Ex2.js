 ...
 * Exercício 2
 * ------------
 * Crie uma função que retorne todos os personagens POR um tipo.
 */

// function tiposDePersonagem() {
//     let tipos = [];
//     personagens.map((personagem) =>
//         tipos.indexOf(personagem.type) == -1 ? tipos.push(personagem.type) : ""
//     );
//     return tipos;
// }
// console.log(tiposDePersonagem());

const forType = (lista, tipo) => {
   const nomes = [];
   lista.forEach(item => {
      item.type.toLowerCase() === tipo.toLowerCase() ?  nomes.push({...item}) : '';
   });
   return nomes;
};
